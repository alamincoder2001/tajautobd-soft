					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<div class="col-sm-7">
									<v-select v-bind:options="suppliers" v-model="selectedSupplier" v-on:input="onChangeSupplier" label="display_name" id="supplier"></v-select>
								</div>
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-sm-7">
										<v-select v-bind:options="products" v-model="selectedProduct" label="display_text" v-on:input="onChangeProduct" id="product"></v-select>
									</div>
									<div class="col-sm-1" style="padding: 0;">
								...
								<div class="form-group">
									...
									<div class="col-sm-3">
										<input type="text" id="purchaseRate" name="purchaseRate" class="form-control" placeholder="Pur. Rate" v-model="selectedProduct.A_Product_Purchase_Rate" v-on:input="productTotal" />
									</div>

									<label class="col-sm-2 control-label no-padding-right"> Quantity </label>
									<div class="col-sm-3">
										<input type="text" step="0.01" id="quantity" name="quantity" class="form-control" placeholder="Quantity" ref="quantity" v-model="selectedProduct.quantity" v-on:input="productTotal" />
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type="text" id="sellingPrice" name="sellingPrice" class="form-control" v-model="selectedProduct.A_Product_SellingPrice" />
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<button type="submit" class="btn btn-default pull-right" id="add2cart" v-on:click="openPOnClick">Add
											Cart</button>
									</div>
								</div>
							</form>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Transport /
													Labour Cost</label>
												<div class="col-sm-12">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Previous
													Due</label>
												<div class="col-sm-12">
...
<script>
	...
	new Vue({
		...
		async created() {
			...
			document.querySelector('#supplier input[role="combobox"]').focus();
		},
		methods: {
			getBranches() {
			...
			onChangeSupplier() {
				...
				if (this.purchase.purchaseId != 0 && this.oldSupplierId != parseInt(this.selectedSupplier
						.Supplier_SlNo)) {
					let changeConfirm = confirm(
						'Changing supplier will set previous due to current due amount. Do you really want to change supplier?'
					);
					if (changeConfirm == false) {
						return;
					}
				} else if (this.purchase.purchaseId != 0 && this.oldSupplierId == parseInt(this.selectedSupplier
						.Supplier_SlNo)) {
			...
			addToCart() {
				...
				this.calculateTotal();
				document.querySelector('#add2cart').focus();
			},
			openPOnClick() {
			...
			calculateTotal() {
				...
				this.purchase.total = (parseFloat(this.purchase.subTotal) + parseFloat(this.purchase.vat) +
					parseFloat(this.purchase.freight)) - this.purchase.discount;
				if (this.selectedSupplier.Supplier_Type == 'G') {
					...
					this.purchase.due = (parseFloat(this.purchase.total) - parseFloat(this.purchase.paid)).toFixed(
						2);
				}
			},
			savePurchase() {
			...
			getPurchase() {
				axios.post('/get_purchases', {
					...
					this.selectedSupplier.display_name = purchase.Supplier_Type == 'G' ?
						'General Supplier' : `${purchase.Supplier_Code} - ${purchase.Supplier_Name}`;
